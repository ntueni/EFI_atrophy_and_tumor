...
#ifndef SRC_EFI_INCLUDE_EFI_CONSTITUTIVE_NEO_HOOKE_ALTERNATIVE_H_
#define SRC_EFI_INCLUDE_EFI_CONSTITUTIVE_NEO_HOOKE_ALTERNATIVE_H_
...
/// Alternative implementation of the neo-Hookean material model. For given
/// deformations, it computes the Piola stresses and its tangent with respect
/// to the deformation gradient. The free energy $\psi := \psi(\boldsymbol{F})$
/// is given by $\psi(\boldsymbol{F}) = \frac{1}{2}[\overline{\boldsymbol{F}}:
/// \overline{\boldsymbol{F}}-3] + frac{1}{4}[J^2-1-2\,\ln J]$, where
/// $\overline{\boldsymbol{F}} = J^{-1/\text{\tt dim]}}\boldsymbol{F}$ denotes
/// the deviatoric part of the deformation gradient.
/// @author Stefan Kaessmair
template <int dim>
class NeoHookeAlternative : public ConstitutiveBase<dim>
{
public:
    ...
    /// of the to-be-constructed (alternative) neo-Hooke constitutive model and
    /// its end.
    NeoHookeAlternative (const std::string &subsection_name,
                         const std::string &unprocessed_input);

    /// Default destructor.
    ~NeoHookeAlternative () = default;
...
parse_parameters (dealii::ParameterHandler &prm)
{
    ...
    efilog(Verbosity::verbose) << "NeoHooke constitutive model finished parsing "
                                  "parameters."
                               << std::endl;
}
...
#endif /* SRC_EFI_INCLUDE_EFI_CONSTITUTIVE_NEO_HOOKE_ALTERNATIVE_H_ */
