/*
 * Copyright (C) 2019 - 2020 by the emerging fields initiative 'Novel Biopolymer
 * Hydrogels for Understanding Complex Soft Tissue Biomechanics' of the FAU
 *
 * This file is part of the EFI library.
 *
 * The EFI library is free software; you can use it, redistribute
 * it, and/or modify it under the terms of the GNU Lesser General
 * Public License as published by the Free Software Foundation; either
 * version 2.1 of the License, or (at your option) any later version.
 *
 * Author: Stefan Kaessmair
 */

#ifndef SRC_MYLIB_INCLUDE_EFI_CONSTITUTIVE_CONSTITUTIVE_FACTORY_H_
#define SRC_MYLIB_INCLUDE_EFI_CONSTITUTIVE_CONSTITUTIVE_FACTORY_H_

// efi headers
#include <efi/base/exceptions.h>
#include <efi/base/utility.h>
#include <efi/constitutive/modified_one_term_ogden.h>
#include <efi/constitutive/maxwell_element.h>
#include <efi/constitutive/maxwell_wiechert.h>
#include <efi/constitutive/neo_hooke.h>
#include <efi/constitutive/neo_hooke_alternative.h>
#include <efi/constitutive/ogden.h>
#include <efi/base/factory_tools.h>


namespace efi
{


// Factory pattern implementation for
// constitutive models.
template <int dim>
class ConstitutiveFactory
{
public:

    // Create a new constitutive model instance of
    // specified by the "type" stored in the
    // Sepcifications.
    static
    ConstitutiveBase<dim>*
    create (const std::vector<std::string>     &section_path,
            const FactoryTools::Specifications &specs,
            const std::string                  &unprocessed_input);

    // Create a new constitutive model instance of
    // specified by the string type_str.
    // The arguments args... are forwarded to the
    // contitutive model constructor.
    // An error of type efi::ExcNotConstructible
    // is thrown if the given string does not match
    // any option.
    template <class ... Args>
    static
    ConstitutiveBase<dim>*
    create (const std::string& type_str,
            Args &&... args);

    // return the keyword which
    static
    std::string
    keyword ();

    // Return a string that specifies the list of
    // of allowed options. The options are separated
    // by '|' no addional spaces are added (e.g.
    // "model1|model2|model3").
    static
    std::string
    get_names ();
};



//---------------------- INLINE AND TEMPLATE FUNCTIONS -----------------------//



template <int dim>
inline
ConstitutiveBase<dim>*
ConstitutiveFactory<dim>::
create (const std::vector<std::string>     &section_path,
        const FactoryTools::Specifications &specs,
        const std::string                  &unprocessed_input)
{
    return ConstitutiveFactory<dim>::create (
            specs.get("type"),
            get_section_path_str(section_path) + "/" +
                FactoryTools::get_subsection_name (keyword(),specs),
            unprocessed_input);
}



template <int dim>
template <class ... Args>
inline
ConstitutiveBase<dim>*
ConstitutiveFactory<dim>::
create (const std::string& type_str,
        Args &&... args)
{
    
    if (type_str == "neo_hooke")
    {
        using model_type = NeoHooke<dim>;

        return make_new_if_constructible<model_type>(std::forward<Args>(args)...);
    }
    if (type_str == "neo_hooke_alternative")
    {
        using model_type = NeoHookeAlternative<dim>;

        return make_new_if_constructible<model_type>(std::forward<Args>(args)...);
    }
    else if (type_str == "ogden")
    {
        using model_type = Ogden<dim>;

        return make_new_if_constructible<model_type>(std::forward<Args>(args)...);
    }
    else if (type_str == "modified_one_term_ogden")
    {
        using model_type = ModifiedOneTermOgden<dim>;

        return make_new_if_constructible<model_type>(std::forward<Args>(args)...);
    }
    else if (type_str == "maxwell_element")
    {
        using model_type = MaxwellElement<dim>;

        return make_new_if_constructible<model_type>(std::forward<Args>(args)...);
    }
    else if (type_str == "maxwell_wiechert")
    {
        using model_type = MaxwellWiechert<dim>;

        return make_new_if_constructible<model_type>(std::forward<Args>(args)...);
    }
    else
        AssertThrow (false, ExcNotConstructible ());
}



template <int dim>
inline
std::string
ConstitutiveFactory<dim>::
keyword ()
{
    return "constitutive";
}



template <int dim>
inline
std::string
ConstitutiveFactory<dim>::
get_names ()
{
    return "neo_hooke|neo_hooke_alternative|ogden|modified_one_term_ogden|maxwell_element|"
           "maxwell_wiechert";
}

}//namespace efi


#endif /* SRC_MYLIB_INCLUDE_EFI_CONSTITUTIVE_CONSTITUTIVE_FACTORY_H_ */
